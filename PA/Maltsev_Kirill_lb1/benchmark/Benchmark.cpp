...
extern const int ITERATIONS = 16;
...
BENCHMARK_DEFINE_F(BenchmarkMatrixMultiplier, MultiplyAsync)(benchmark::State& state)
{
    for (auto _ : state)
    {
        multiplyAsync(*a_ptr, *b_ptr);
    }
}
...
BENCHMARK_REGISTER_F(BenchmarkMatrixMultiplier, MultiplyAsync)
    ->DenseRange(128, 2048, 128)
    ->Iterations(ITERATIONS)
    ->Unit(benchmark::kMillisecond);
