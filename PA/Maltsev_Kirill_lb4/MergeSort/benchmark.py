...
df['algorithm'] = df['name'].str.extract(r'/(MergeSort|DnCParallelMergeSort|TaskParallelMergeSort|CudaMergeSort)/')
df['elements'] = df['name'].str.extract(r'/(\d+)/').astype(int)

plt.figure(figsize=(10, 6))

for algo in ['MergeSort', 'DnCParallelMergeSort', 'TaskParallelMergeSort', 'CudaMergeSort']:
